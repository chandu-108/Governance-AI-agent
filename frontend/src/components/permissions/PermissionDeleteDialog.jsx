...
const PermissionDeleteDialog = ({ open, onClose, onConfirm, permission, isLoading }) => (
  <AnimatePresence>
    {open && (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          <div className="p-6">
            ...
            <h2 className="text-base font-semibold text-gray-900 text-center mb-1">Revoke Permission</h2>
            <p className="text-sm text-gray-500 text-center mb-6 leading-relaxed">
              Are you sure you want to revoke{' '}
              <span className="font-semibold text-gray-700">{permission?.permission}</span>{' '}
              access for Agent #{permission?.agent_id}? This action cannot be undone.
            </p>
            <div className="flex gap-3">
              <button onClick={onClose} className="flex-1 px-4 py-2.5 text-sm font-medium text-gray-700 bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors">
                Cancel
              </button>
              <button
                onClick={onConfirm}
